...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((keyword) =>
      keyword
        .setName("keyword")
        .setDescription("Word(s) to add to the whitelist")
        .setRequired(false),
    ),
  ...
  async execute(message) {
    ...
    if (typeof words[0] === "string") {
      ...
      let list = "[";
      functions
        .liofaRead(message.guild.id)
        .Settings.whitelist.forEach(
      ...
      const whiteEmbed = new EmbedBuilder()
        ...
        .setFooter(
          "Use " +
            prefix +
            "whitelist <words to add/remove> to edit the whitelist",
        );
    ...
    async function whitelistToggle(interaction, toggleList) {
      const commandReply = await interaction.channel.send(
        "Editing Whitelist...",
      );
      let response = "";
      for (let i = 0; i < toggleList.length; i++) {
        const listLength = GuildData.Settings.whitelist.length;
        GuildData.Settings.whitelist = functions.arrayToggle(
          GuildData.Settings.whitelist,
          toggleList[i],
        );
      ...
      let list = "[";
      GuildData.Settings.whitelist.forEach(
        (element) => (list = list + element + "], ["),
      );
      list = list.slice(0, -3);
