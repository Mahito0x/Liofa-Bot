...
module.exports = {
	data: new SlashCommandBuilder()
		...
		.addSubcommand((subcommand) =>
			subcommand.setName('clear').setDescription('Clears the modlog channel'),
		),
	...
	async execute(interaction) {
		...
		if (inputs.getChannel('channel')) {
			const channel = inputs.getChannel('channel').id;
			GuildData.Settings.modlog = channel;
			interaction.reply(
				'My moderation actions will be sent to <#' + channel + '>',
			);
		}
		else if (inputs._subcommand === 'clear') {
			const buttons = new ActionRowBuilder().addComponents(
				new ButtonBuilder()
					.setCustomId('modlog confirm')
					.setLabel('Confirm')
					.setStyle(ButtonStyle.Success),
				new ButtonBuilder()
					.setCustomId('modlog cancel')
					.setLabel('Cancel')
					.setStyle(ButtonStyle.Danger),
			);
			await interaction.reply({
				content: 'Are you sure you want to deactivate the modlog?',
				components: [buttons],
			});
			return;
		}
