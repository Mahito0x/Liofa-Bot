...
async function displayPerms(interaction, permToDisplay) {
  ...
  if (permToDisplay === "all") {
    for (const [value] of Object.entries(GuildData.Permissions)) {
      for (let i = GuildData.Permissions[value].length - 1; i >= 0; i--) {
        GuildData.Permissions[value][i] = functions.roleToString(
          GuildData.Permissions[value][i],
          interaction,
        );
      }
    }
    ...
    for (let i = GuildData.Permissions[permToDisplay].length - 1; i >= 0; i--) {
      GuildData.Permissions[permToDisplay][i] = functions.roleToString(
        GuildData.Permissions[permToDisplay][i],
        interaction,
      );
    }
  ...
  list =
    "__**" +
    functions.capitalizeFirstLetter(permToDisplay) +
    " Permissions**__ \n >>> " +
    list;
  return list;
}
...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) => {
      option
        .setName("command")
        .setDescription("A command to edit the permissions for")
        .setRequired(false);
    ...
    .addRoleOption((role) =>
      role
        .setName("role0")
        .setDescription("A role to add/remove from the command permissions"),
    )
    .addRoleOption((role) =>
      role
        .setName("role1")
  ...
  async execute(interaction) {
    ...
    if (args[0] && permission) {
      ...
      interaction.channel.send(
        "🤡 Please provide a permission AND a role if you would like to edit the permissions",
      );
    } else {
      permission = "all";
    }
    ...
    async function permToggle(perm, roles) {
      ...
      // Used for toggling what roles can use what commands
        ...
        if (!roles.every((role) => interaction.guild.roles.cache.has(role)))
          return interaction.reply(
            "One or more of the given roles do not exist",
          );
        ...
        for (let i = 0; i < roles.length; i++) {
          ...
          GuildData.Permissions[perm] = functions.arrayToggle(
            GuildData.Permissions[perm],
            roles[i],
          );
          if (permLength > GuildData.Permissions[perm].length) {
            response =
              response +
              "\n❌ " +
              functions.roleToString(roles[i], interaction) +
              " removed from " +
              perm;
          } else {
            response =
