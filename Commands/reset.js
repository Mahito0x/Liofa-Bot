...
module.exports = {
  ...
  async execute(interaction) {
    ...
    await interaction.reply({
      content: "Are you sure you want to reset your settings?",
      components: [buttons],
    });
    return;
  },
  buttons: {
    confirm: async function confirm(interaction) {
      const tempSettings = JSON.parse(
        fs.readFileSync("Read Only/Settings.json"),
      );
      fs.writeFileSync(
      ...
      console.log(
        "Created settings file for " + interaction.guild.id.toString(),
      );
