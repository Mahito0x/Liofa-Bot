...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addChannelOption((channel) =>
      channel
        .setName("whitelist")
        .setDescription("A channel to add to the whitelist")
        .setRequired(false),
    )
    .addStringOption((keyword) =>
      keyword
        .setName("keyword")
        .setDescription("All channels with this keyword will be ignored")
        .setRequired(false),
    ),
  ...
  execute(message) {
    ...
    const inputCount = functions.onlyOne([
      inputs.getChannel("whitelist"),
      inputs.getString("keyword"),
    ]);
    ...
    async function info(interaction) {
      ...
      let list = "";
      GuildData.Settings.channels.forEach(
        (element) =>
          (list =
            list + functions.channelToString(element, interaction) + "\n> "),
      );
      list = list.slice(0, -2);
      let channelList = "__**Whitelisted Channels:**__ \n> " + list;
      list = "[";
      GuildData.Settings.channelIgnore.forEach(
    ...
    async function whitelist(interaction, channels) {
      if (channels.length == 0) {
        interaction.channel.send(
          "🤡 No channels given, please provide at least one channel",
        );
        return;
      }
      ...
      if (
        !channels.every((ch) => {
          const LookUpChannel = interaction.guild.channels.cache.find(
            (channel) => channel.id === ch,
          );
          return (
            interaction.guild.channels.cache.has(ch) &&
            !bannedChannelTypes.includes(LookUpChannel.type)
          );
        })
      ) {
        interaction.channel.send(
          "🤡 One or more channels do not exist or are not accepted channel types",
        );
        return;
      } else {
        channels.forEach(
          (ch) =>
            (GuildData.Settings.channels = functions.arrayToggle(
              GuildData.Settings.channels,
              ch,
            )),
        );
    ...
    async function keyword(interaction, ignore) {
      if (ignore.length == 0) {
        interaction.channel.send(
          "No words to ignore given, please provide at least one word",
        );
        return;
      }
      for (let i = 0; i < ignore.length; i++) {
        GuildData.Settings.channelIgnore = functions.arrayToggle(
          GuildData.Settings.channelIgnore,
          ignore[i],
        );
      }
