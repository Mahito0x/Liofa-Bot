...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addBooleanOption((trans) =>
      trans.setName("translator").setDescription("Link to Google Translate"),
    )
    .addBooleanOption((lang) =>
      lang.setName("language").setDescription("What language is being spoken"),
    )
    .addBooleanOption((undo) =>
      undo.setName("undo").setDescription("Undo infraction"),
    )
    .addBooleanOption((get) =>
      get.setName("get").setDescription("Link to Liofa"),
    ),
  ...
  async execute(interaction) {
    ...
    const buttons = await [
      interaction.options.getBoolean("translator"),
      interaction.options.getBoolean("language"),
      interaction.options.getBoolean("undo"),
      interaction.options.getBoolean("get"),
    ];
