...
async function displayInfractions(interaction, target) {
  ...
  const timeSinceLastInfraction = functions.minutesSince(
    Date.now(),
    GuildData.Watchlist[target.id].time,
  );
  ...
  const infractionsEmbed = new EmbedBuilder()
    ...
    .addFields(
      {
        name: "Minutes since last infraction",
        value: bold(timeSinceLastInfraction),
      },
...
async function modButtons(resetButton, target) {
  const display = new ActionRowBuilder().addComponents(
    new ButtonBuilder()
  ...
  async execute(message) {
    ...
    if (functions.liofaPrefixCheck(message)) {
      ...
      target = await message.guild.members.fetch(
        functions.userToID(args[1], message),
      );
      target = target.user;
    ...
    async function info(interaction) {
      ...
      if (GuildData.Watchlist[target.id] != null) {
        ...
        return interaction.reply({
          content: target.username + " has 0 infractions",
          components: [buttons],
        });
      }
    }
  ...
  buttons: {
    reset: async function reset(interaction, name) {
      ...
      let target = await interaction.guild.members.fetch(
        functions.userToID(name[2], interaction),
      );
      target = target.user;
      if (typeof target === "undefined") return;
      ...
      interaction.channel.send(
        target.username + "'s infractions have been reset",
      );
    ...
    undo: async function undo(interaction, name) {
      ...
      message.delete();
      return interaction.reply({
        content: target.username + " has one less infraction",
        ephemeral: true,
      });
    },
